...
import (
	"bytes"
	"crypto/tls"
	"github.com/instana/instana-agent-operator/controllers/reconciliation/helm/helper/version"
...
func (g *HTTPGetter) httpClient() (*http.Client, error) {
	...
	if len(g.opts.proxy) > 0 {
		proxyURL, err := url.Parse(g.opts.proxy)
		if err != nil {
			// TODO log error
			//fmt.Errorf("cannot parse proxy URL: %w", err)
		} else {
